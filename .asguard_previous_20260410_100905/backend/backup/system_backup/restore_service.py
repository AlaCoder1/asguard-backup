...
class RestoreService:
    ...
    def restore_full_safe(cls, backup_id: str) -> dict:
        """
        Restore standard:
        - restore VM/config/data/services
        - DO NOT restore application code
        - keep current running code
        """
        return cls._restore_full(backup_id, include_application=False)

    @classmethod
    def restore_full_complete(cls, backup_id: str) -> dict:
        """
        Disaster recovery restore:
        - restore everything including application
        """
        return cls._restore_full(backup_id, include_application=True)

    @classmethod
    def restore_full(cls, backup_id: str) -> dict:
        """
        Backward compatibility:
        default full restore = SAFE mode
        """
    ...
    @classmethod
    def _restore_full(cls, backup_id: str, include_application: bool) -> dict:
        ...
        return {
            ...
            "backup_id": backup_id,
            "mode": "full_complete" if include_application else "full_safe",
            "results": results,
            "summary": {
                "success": success,
                "failed": failed,
