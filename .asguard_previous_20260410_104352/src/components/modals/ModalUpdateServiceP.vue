<template>
  <v-row justify="center">
    <v-dialog v-model="state.openModal" persistent width="600">
      <form ref="myForm" @submit.prevent="updateServiceP">
        <v-card>
          <v-card-title>
            <span class="text-h5">{{ $t("ztna.updateServicePolicy") }}</span>
          </v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
  ...
  setup(props) {
    ...
    const updateServiceP = async () => {
      try {
        ...
        if (serviceRA.value.trim() !== "") {
          requestBody.serviceRA = serviceRA.value;
        }
        if (identityatt.value.trim() !== "") {
          requestBody.identityatt = identityatt.value;
        }
        if (selectedsemantic.value.trim() !== "") {
          requestBody.selectedsemantic = selectedsemantic.value;
        }
        const proxyUrl = "https://asguard:3000";
        const apiUrl = `/edge/management/v1/service-policies/${state.itemId}`;
