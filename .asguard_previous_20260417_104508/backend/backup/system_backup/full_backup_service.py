...
3) create_components_backup(component_names)
   -> Backup personnalisé d'un ou plusieurs composants

4) create_component_backup(component_name)
   -> Alias pratique pour un seul composant
"""
...
class FullBackupService:
    ...
    @classmethod
    def _component_backup_registry(cls) -> dict[str, callable]:
        return {
    ...
    @classmethod
    def _all_component_names(cls) -> list[str]:
        return list(cls._component_backup_registry().keys())

    @classmethod
    def _safe_component_names(cls) -> list[str]:
        return [
            "firewall",
            "vpn",
            "ids",
    ...
    @classmethod
    def _prepare_backup_subdirs(cls, backup_dir: Path, component_names: list[str]) -> None:
        for sub in component_names:
            (backup_dir / sub).mkdir(parents=True, exist_ok=True)

    @classmethod
    def _resolve_runners(cls, component_names: list[str]) -> list[callable]:
        registry = cls._component_backup_registry()
        return [registry[name] for name in component_names]

    @classmethod
    def _validate_component_names(cls, component_names: list[str]) -> tuple[list[str], list[str]]:
        registry = cls._component_backup_registry()
        cleaned = []
        invalid = []
    ...
    def create_full_backup(cls) -> dict:
        ...
        component_names = cls._all_component_names()
        cls._prepare_backup_subdirs(backup_dir, component_names)

        runners = cls._resolve_runners(component_names)
    ...
    def create_safe_backup(cls) -> dict:
        ...
        component_names = cls._safe_component_names()
        cls._prepare_backup_subdirs(backup_dir, component_names)
    ...
    @classmethod
    def create_components_backup(cls, component_names: list[str], backup_prefix: str = "backup_custom") -> dict:
        valid_components, invalid_components = cls._validate_component_names(component_names)
        ...
        if not valid_components:
            return {
                ...
                "available_components": cls._all_component_names(),
            }

        timestamp = datetime.now(LOCAL_TZ).strftime("%Y-%m-%d_%H-%M-%S")
        backup_id = f"{backup_prefix}_{timestamp}"
        backup_dir = cls.BACKUP_ROOT / backup_id
        backup_dir.mkdir(parents=True, exist_ok=True)

        cls._prepare_backup_subdirs(backup_dir, valid_components)

        runners = cls._resolve_runners(valid_components)
        results = cls._run_backup_runners(backup_dir, runners)

        payload = cls._finalize_backup(
            backup_id=backup_id,
            backup_dir=backup_dir,
            results=results,
        ...
        payload["invalid_components"] = invalid_components
        return payload

    @classmethod
    def create_component_backup(cls, component_name: str) -> dict:
        return cls.create_components_backup([component_name], backup_prefix="backup_component")
