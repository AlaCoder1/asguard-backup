...
urlpatterns = [
    ...
    path("create-components-backup", views.create_components_backup, name="createComponentsBackup"),

    path("<str:backup_id>/details", views.get_backup_details, name="getBackupDetails"),

    # SAFE full restore = sans application
    path("<str:backup_id>/restore", views.restore_backup, name="restoreBackup"),

    # COMPLETE full restore = avec application
    path("<str:backup_id>/restore-full", views.restore_full_backup, name="restoreFullBackup"),

    # SAFE restore selected components
    path("<str:backup_id>/restore-components", views.restore_components_backup, name="restoreComponentsBackup"),

    # COMPLETE restore selected components
    path(
        "<str:backup_id>/restore-components-complete",
        views.restore_components_complete_backup,
        name="restoreComponentsCompleteBackup",
    ),
