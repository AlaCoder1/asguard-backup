  ...
  <div class="backup-panel">
    ...
    <transition name="restore-monitor-fade">
      <div v-if="restoreMonitor.visible" class="restore-monitor" :class="restoreMonitor.status">
        ...
        <div v-if="restoreMonitor.progressActive" class="restore-progress">
...
export default {
  ...
  methods: {
    ...
    openRestoreMonitor({ backupId, modeLabel, title, subtitle, status = "running", statusLabel = "Running", progressActive = true, verification = null }) {
      this.restoreMonitor = {
        visible: true,
        title,
        subtitle,
        ...
        try {
          ...
          this.openRestoreMonitor({
            ...
            subtitle: isFinished
              ...
              : `Job ${jobId} actif. On attend la fin et les indices de verification.`,
            status,
    ...
    exportBackup(backup) {
      ...
      window.location.href = `/backup/${backup.id}/export`;
    },
    triggerImport() {
    ...
    async uploadImport(file) {
      if (file.size > 2 * 1024 * 1024 * 1024) {
        this.notify("Le fichier dépasse 2 GB.", "error");
        return;
      }
      ...
      try {
        ...
        this.notify(
          error.response?.data?.message || "Erreur lors de l'import du backup.",
          "error"
        );
      } finally {
        this.loading = false;
