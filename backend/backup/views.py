...
from .system_backup.safe_restore_service import SafeRestoreService
        ...
        if d.is_dir():
            ...
            if meta_file.exists():
                try:
                    ...
                    backup_type = "safe" if backup_scope == "safe_restore_ui" else "full"
...
@swagger_auto_schema("POST", responses={200: "OK"}, operation_summary="SAFE RESTORE BACKUP (ADMIN UI)")
@api_view(["POST"])
@require_http_methods(["POST"])
@authentication_classes([SessionAuthentication])
@permission_classes([AllowAny])
def restore_backup(request, backup_id):
    result = SafeRestoreService.restore_safe(backup_id)
...
@swagger_auto_schema("POST", responses={202: "Accepted"}, operation_summary="FULL RESTORE BACKUP (DISASTER RECOVERY)")
@api_view(["POST"])
@require_http_methods(["POST"])
@authentication_classes([SessionAuthentication])
@permission_classes([AllowAny])
def restore_full_backup(request, backup_id):
    ...
    job_id = f"restore_full_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{backup_id}"
    ...
    unit_name = f"asguard-restore-{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    cmd = [
        ...
        "systemd-run",
        "--unit", unit_name,
        "--description", f"Asguard full restore {backup_id}",
    ...
    try:
        ...
        logger.info("Full restore unit started: %s | output=%s", unit_name, launch.stdout.strip())

        return JsonResponse({
            "status": "started",
            "message": "Full restore launched in detached systemd unit.",
            "backup_id": backup_id,
            "job_id": job_id,
        ...
        logger.exception("Failed to launch full restore for %s", backup_id)
        return JsonResponse({"status": "error", "message": str(e)}, status=500)
