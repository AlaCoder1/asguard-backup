...
def create_private_key(private_key_name, private_key_length):
    ...
    execute_command_without_arguments(['sudo', 'openssl', 'genrsa', '-out', f'/etc/ipsec.d/private/{private_key_name}.pem', private_key_length])


def delete_private_key_in_system(private_key_name):
    """Function to delete a private in system"""
    execute_command_without_arguments(['sudo', 'rm', '-rf', f'/etc/ipsec.d/private/{private_key_name}.pem'])


def create_public_key(private_key_name, public_key_name):
    """Function to create in system a private key"""
    commands_list_without_arguments = [['mkdir', '-p', '/etc/ipsec.d/finger_prints/'],
                                       ['sudo', 'openssl', 'rsa', '-in', f'/etc/ipsec.d/private/{private_key_name}.pem', '-pubout', '-out', f'/etc/ipsec.d/certs/{public_key_name}.pem'],
                                       ]
...
def import_public_key(public_key_name, public_key_value):
    ...
    with open(f'/etc/ipsec.d/certs/{public_key_name}.pem', 'w') as public_key_file:
...
def delete_public_key_in_system(public_key_name):
    ...
    execute_command_without_arguments(['sudo', 'rm', '-rf', f'/etc/ipsec.d/certs/{public_key_name}.pem'])
