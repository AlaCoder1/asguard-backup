...
from backend.nat.utils import get_next_nat_handle, update_position_nat
...
def start_snat(request, id):
    ...
    try:
        ...
        rule_number = create_snat_rule_in_system(snat.interface.ifname, source, destination, snat.protocol, masking, position_insert,
...
def start_one_to_one_nat(request, id):
    ...
    try:
        ...
        # Add the rule in system
        # Find the next activated rule handle to insert the started rule above
        list_next_one_to_one_nat = SNat.objects.filter(rule_status=True, one_to_one_nat_position__gt=one_to_one_nat.one_to_one_nat_position)
        position_insert = 0
        postrouting_position = 0
