...
# Check if required environment variables are set and then run the Ziti controller in background detached mode
if [ -n "$ZITI_BIN_DIR" ] && [ -n "$ZITI_HOME" ] && [ -n "$ZITI_CTRL_NAME" ]; then
    nohup "${ZITI_BIN_DIR-}/ziti" controller run "${ZITI_HOME}/${ZITI_CTRL_NAME}.yaml" &>/dev/null &
    disown $!
else
