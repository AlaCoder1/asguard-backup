<template>
    <v-app id="inspire">
      <base-layout title="Intrusion Detection" active-menu="CONFIGURATION">
        <template #content>
          ...
          <v-window v-model="activeTab">
            <v-window-item v-for="tab in tabs" :key="tab.id" value="CONFIGURATION">
              <v-card>
            ...
            <v-window-item v-for="tab in tabs" :key="tab.id" value="RULES">
              <v-card>
                <v-card-text> <RulesComponent :configInfo="configurationInfo"/></v-card-text>
              </v-card>
            </v-window-item>
  ...
  export default {
    ...
    mounted: async function () {
      ...
      this.rowDataConfiguration =
        document.getElementById("app").attributes["general_config_suricata"].value;
